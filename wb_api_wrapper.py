...
def mlr_year(data,least_or_most):    
    """year of LEAST or MOST recent values from a dataframe. assumes one column is called 'year' """
    try:
        if data.shape[1]>1:
            data = data.unstack()
    except IndexError :
    ...
    elif least_or_most=="least":
        return data.reset_index().dropna().groupby("country")["year"].min()
    else:
        raise ValueError('least_or_most was not in ["least","most"]')
        
def lr_year(data):    
    """year of MOST recent values from a dataframe. assumes one column is called 'year' """
    return  mlr_year(data,"least") 
    
def mr_year(data):    
    """year of MOST recent values from a dataframe. assumes one column is called 'year' """
    return  mlr_year(data,"most") 
